...
init = mice(species_traits2,maxit=50,method="pmm")
meth = init$method
...
imputed <- mice(data = species_traits2, method="pmm", m=1000, predictorMatrix = predM) 
...
is_missing <- is.na(species_traits2[,-c(1)])
...
comparisondf$speciesName <- species_traits2$species[comparisondf$species_id]
...
all_traits <- ggplot(comparisondf, aes(x = speciesName)) + facet_wrap(~ trait_id, scales = 'free',labeller = labeller(trait_id = traitNames)) +
  ...
  ggtitle("Imputations with 95% CI at 25% Missing Values using Training Dataset\n") +
